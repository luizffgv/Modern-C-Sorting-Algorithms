...
char *Part_(size_t elem_sz, Range range, char *pivot_data, Comparer comp);
static void QuickSort_(size_t elem_sz, Range range,
                       char *pivot_data, Comparer comp);
...
#define RESERVED_PIVOT_DATA_SZ 64
    ...
    char *pivot_data = elem_sz > RESERVED_PIVOT_DATA_SZ ? malloc(elem_sz)
                                                        : reserved_pivot_data;

    QuickSort_(elem_sz, (Range){range.begin, (char *)range.end - elem_sz},
               pivot_data, comp);
                       ...
                       char *pivot_data, Comparer comp)
{
    ...
    {
        char *middle = Part_(elem_sz, range, pivot_data, comp);
...
inline char *Part_(size_t elem_sz, Range range, char *pivot_data, Comparer comp)
{
    ...
    while (1)
    {
        while (range.begin < range.end
               && comp(pivot_data, range.end) != ordering_greater)
            range.end = (char *)range.end - elem_sz;
        if (range.begin >= range.end)
            break;
        memcpy(range.begin, range.end, elem_sz);
        range.begin = (char *)range.begin + elem_sz;

        while (range.begin < range.end
               && comp(range.begin, pivot_data) != ordering_greater)
            range.begin = (char *)range.begin + elem_sz;
        if (range.begin >= range.end)
            break;
